...
def build_trade_signal(
    ...
    latest_date = pd.Timestamp(daily_weights.index[-1])
    latest_weights = daily_weights.iloc[-1]
    latest_prices = prices.loc[latest_date]
    latest_rebalance = rebalance_log.iloc[-1].to_dict() if not rebalance_log.empty else {}
    targets = []
    for code, name in ETF_UNIVERSE.items():
        if code not in latest_weights.index or code not in latest_prices.index:
            continue
        weight = float(latest_weights.get(code, 0.0))
        price = latest_prices.get(code)
        targets.append(
            {
                "code": code,
                "futu_code": futu_symbol(code),
                "name": name,
                "target_weight": weight,
                "latest_price": None if pd.isna(price) else float(price),
                "selected": weight > 0,
            }
        )
    return {
        ...
        "suggested_rebalance_date": (
            pd.Timestamp(latest_rebalance["date"]).strftime("%Y-%m-%d")
            if latest_rebalance and latest_rebalance.get("date") is not None
        ...
        "market": "CN",
        "config": config_to_dict(config),
        "selected": [item["futu_code"] for item in targets if item["selected"]],
        "targets": targets,
    }
...
def save_outputs(results: Dict[str, pd.DataFrame | pd.Series | dict], config: BacktestConfig, load_result: PriceLoadResult) -> None:
    ...
    trade_signal = build_trade_signal(prices, daily_weights, rebalance_log, config, load_result.actual_data_mode)
    ...
    (OUTPUT_DIR / "trade_signal.json").write_text(
